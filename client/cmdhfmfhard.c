...
#include "cmdhfmfhard.h"
 ...
 const float p_K[257] = {		// the probability that a random nonce has a Sum Property == K
...
typedef struct {
 ...
 uint8_t best_first_bytes[256];
 uint16_t first_byte_Sum = 0;
 uint16_t first_byte_num = 0;
 uint16_t num_good_first_bytes = 0;
...
void free_nonce_list(noncelistentry_t *p)
{
	if (p == NULL) {
		return;
	} else {
...
uint16_t PartialSumProperty(uint32_t state, odd_even_t odd_even)
{ 
	uint16_t sum = 0;
...
double p_hypergeometric(uint16_t N, uint16_t K, uint16_t n, uint16_t k)
{
...
float sum_probability(uint16_t K, uint16_t n, uint16_t k)
{
...
inline uint_fast8_t common_bits(uint_fast8_t bytes_diff)
{
	 const uint_fast8_t common_bits_LUT[256] = {
...
void Tests()
{
 ...
 void sort_best_first_bytes(void)
{
 ...
 uint16_t estimate_second_byte_sum(void)
{
 ...
 int read_nonce_file(void)
{
 ...
 void Check_for_FilterFlipProperties(void)
{
 ...
 void simulate_MFplus_RNG(uint32_t test_cuid, uint64_t test_key, uint32_t *nt_enc, uint8_t *par_enc)
{
 ...
 void simulate_acquire_nonces()
{
 ...
 int acquire_nonces(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType, bool nonce_file_write, bool slow)
{
	clock_t time1 = clock();
 ...
 int init_partial_statelists(void)
{
 ...
 void init_BitFlip_statelist(void)
{
 ...
 inline bool /*__attribute__((always_inline))*/ invalid_state(uint_fast8_t byte_diff, uint_fast32_t state1, uint_fast32_t state2, uint_fast8_t bit, uint_fast8_t state_bit)
{
 ...
 inline bool remaining_bits_match(uint_fast8_t num_common_bits, uint_fast8_t byte_diff, uint_fast32_t state1, uint_fast32_t state2, odd_even_t odd_even)
{
	if (odd_even) {
		// odd bits
 ...
 bool all_other_first_bytes_match(uint32_t state, odd_even_t odd_even)
{
 ...
 bool all_bit_flips_match(uint32_t state, odd_even_t odd_even)
{
	for (uint16_t i = 0; i < 256; i++) {
 ...
 void init_statelist_cache(void)
{
 ...
 int add_matching_states(statelist_t *candidates, uint16_t part_sum_a0, uint16_t part_sum_a8, odd_even_t odd_even)
{
 ...
 statelist_t *add_more_candidates(statelist_t *current_candidates)
{
 ...
 void TestIfKeyExists(uint64_t key)
{
	struct Crypto1State *pcs;
 ...
 void generate_candidates(uint16_t sum_a0, uint16_t sum_a8)
{
 ...
 void	free_candidates_memory(statelist_t *sl)
{
	if (sl == NULL) {
		return;
	} else {
 ...
 void free_statelist_cache(void)
{
	for (uint16_t i = 0; i < 17; i+=2) {
		for (uint16_t j = 0; j < 17; j+=2) {
			for (uint16_t k = 0; k < 2; k++) {
